...
import { DEFAULT_ACCORDION_STATE } from "./_data/AccordionPresets";
...
import { SECTIONS, type SectionId, type AccordionState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<AccordionState>(DEFAULT_ACCORDION_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("accordion-component");
  ...
  const update = <K extends keyof AccordionState>(key: K, value: AccordionState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_ACCORDION_STATE, ...(preset.state as Partial<AccordionState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Accordion Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
